	...
	public function computeProbabilities($text)
	{
		...
		for($word=0; $word<count($words); $word++)
		{
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_FRENCH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_FRENCH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_FRENCH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_ENGLISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_ENGLISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_ENGLISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_ARABIC]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_ARABIC]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_ARABIC] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_RUSSIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_RUSSIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_RUSSIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_GERMAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_GERMAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_GERMAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_ITALIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_ITALIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_ITALIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_GREEK]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_GREEK]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_GREEK] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_PERSIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_PERSIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_PERSIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_SPANISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_SPANISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_SPANISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_CHINESE]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_CHINESE]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_CHINESE] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_TURKISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_TURKISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_TURKISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_FINNISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_FINNISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_FINNISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_HEBREW]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_HEBREW]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_HEBREW] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_PORTUGUESE]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_PORTUGUESE]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_PORTUGUESE] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_ROMAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_ROMAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_ROMAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_POLISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_POLISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_POLISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_HUNGARIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_HUNGARIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_HUNGARIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_DUTCH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_DUTCH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_DUTCH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_IRISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_IRISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_IRISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_SWEDISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_SWEDISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_SWEDISH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_LATIN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_LATIN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_LATIN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_ICELANDIC]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_ICELANDIC]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_ICELANDIC] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_HINDI]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_HINDI]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_HINDI] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_CZECH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_CZECH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_CZECH] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_MALAYSIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_MALAYSIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_MALAYSIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_BULGARIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_BULGARIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_BULGARIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_NORWEGIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_NORWEGIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_NORWEGIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_ALBANIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_ALBANIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_ALBANIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_URDU]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_URDU]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_URDU] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_THAI]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_THAI]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_THAI] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_INDONESIAN]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_INDONESIAN]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_INDONESIAN] ++;
					break;
				}
					
			for($char_ref=0; $char_ref<count($this->m_language_profiles[LNG_DANISH]->m_vector); $char_ref++)
				if(utf8_strcmp($this->m_language_profiles[LNG_DANISH]->m_vector[$char_ref]->m_element, $words[$word]))
				{
					$probabilities[LNG_DANISH] ++;
					break;
				}
		}
